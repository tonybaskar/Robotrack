import { MapPin, Wrench, Check, ArrowRight } from 'lucide-react'
...
export default function CircuitRail({ items, onStartClass }) {
...
function RailItem({ item, isLast, onStartClass }) {
  const { status } = item // 'completed' | 'current' | 'upcoming'
          ...
          {status !== 'completed' && (
            <span
              className={`h-2 w-2 rounded-full ${
                status === 'current' ? 'bg-white' : 'bg-line'
              }`}
            />
          )}
        </div>
      ...
      <div className={`flex-1 min-w-0 ${isLast ? 'pb-0' : 'pb-4'}`}>
        ...
        >
          ...
          {status !== 'completed' ? (
            ...
            <p className="mt-3 text-xs text-sage font-medium">Class completed</p>
          )}
        </div>
      </div>
    </div>
